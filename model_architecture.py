...
class MultiHeadGuidanceQK(nn.Module):
    ...
    def __init__(self, cfg, num_heads: int, num_hiddens: int, key_dim: int):
        ...
        h = self.dh + nh_kd * 2
        self.qkv_linear = nn.Linear(self.dim, h)
        self.qkv_bn = nn.BatchNorm1d(h)
    ...
    def forward(self, x, nei_inds, pe = None):
        ...
        qkv = self.qkv_bn(qkv.flatten(0, 1)).reshape_as(qkv)
        q, k, v = qkv.view(B, N, self.num_heads, -1).split([self.key_dim, self.key_dim, self.key_dim], dim = 3)
